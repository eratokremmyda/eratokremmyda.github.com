...
jQuery.fn.lavaLamp = function(o) {
	...
	return this.each(function() {
		...
		// creates and adds to the container a backLava element with absolute positioning
	        $back = $('<'+o.container+' class="backLava"><div class="leftLava"></div><div class="bottomLava"></div><div class="cornerLava"></div></'+o.container+'>').css({'position':'absolute','display':'block','margin':0,'padding':0,'opacity':'0.0'}).prependTo(this);
		...
		$(this).bind('mouseleave', function() {
			//console.log('mouseleave');
		        $back.stop(true,true).animate({ opacity: '0.0' });
			return true;
		});
		...
		function move($el) {
			if (!o.includeMargins) {
		        ...
		        var left = $el.position().left+ml, top =  $el.position().top+mt, width = $el.outerWidth()-bx, height = $el.outerHeight()-by;
			var dims = {
				'left': left,
				'top': top,
				'width': width,
		        ...
		        $back.stop();
		        var opacity = $back.css('opacity');
		        if (opacity == '0') $back.css({ top : top, width : width, height : height, left: left });
		        $back.animate(dims, o.speed, o.fx);
		};
	});
